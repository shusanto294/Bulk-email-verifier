...
// Get skip count from command line arguments
const skipCount = parseInt(process.argv[2]) || 0;
const batchSize = parseInt(process.argv[3]) || 10;

console.log(`Worker starting with skip count: ${skipCount}, batch size: ${batchSize}`);
...
async function verifyEmailBatch() {
    try {
        // Find pending emails with skip option
        const emails = await Email.find({ status: 'pending' })
            .skip(skipCount)
            .limit(batchSize);
        
        if (emails.length === 0) {
            console.log(`No pending emails found after skipping ${skipCount}. Waiting for new emails...`);
            await new Promise(resolve => setTimeout(resolve, 2000));
            return;
        }

        console.log(`Processing batch of ${emails.length} emails (skipped ${skipCount})...`);
...
console.log(`Starting email verification worker with skip: ${skipCount}, batch size: ${batchSize}...`);
verifyPendingEmails();
