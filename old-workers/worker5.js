...
mongoose.connect(process.env.MONGODB_URI)
    .then(() => console.log('Worker 5 connected to MongoDB'))
    .catch(err => console.error('MongoDB connection error:', err));

async function verifyEmailBatch() {
    try {
        // Find up to 100 pending emails at a time with offset for worker 5
        const emails = await Email.find({ status: 'pending' }).skip(400).limit(100);
        
        if (emails.length === 0) {
            console.log('Worker 5: No pending emails found. Waiting for new emails...');
            await new Promise(resolve => setTimeout(resolve, 2000));
            return;
        }

        console.log(`Worker 5: Processing batch of ${emails.length} emails...`);
        ...
        for (const email of emails) {
            try {
                ...
                if (!upload || !upload.userId) {
                    console.error(`Worker 5: No user found for email ${email.email}`);
                ...
                if (!user.isAdmin() && user.credits <= 0) {
                    console.log(`Worker 5: User ${user.username} has no credits left. Skipping email verification.`);
                ...
                console.log(`Worker 5: Verifying email: ${email.email} for user: ${user.username} (Role: ${userRole}, Credits: ${user.credits})`);
                ...
                console.log(`Worker 5: Full verifier response for ${email.email}:`, JSON.stringify(result, null, 2));

                if (result.reason === 'timeout') {
                    console.error(`Worker 5: Timeout verifying ${email.email}`);
                    email.status = 'invalid';
                    ...
                    console.log(`Worker 5: Email ${email.email} is ${result.valid ? 'valid' : 'invalid'} - Reason: ${result.reason || 'N/A'}`);
                }
                ...
                if (!user.isAdmin()) {
                    try {
                        ...
                        console.log(`Worker 5: Deducted 1 credit from user ${user.username}. Remaining credits: ${user.credits}`);
                    } catch (creditError) {
                        console.error(`Worker 5: Error deducting credits for user ${user.username}:`, creditError);
                        // If credit deduction fails, mark email as invalid
                        email.status = 'invalid';
                        email.verificationDetails = { error: 'Credit deduction failed' };
                    }
                } else {
                    console.log(`Worker 5: Admin user ${user.username} - No credits deducted`);
                }

                await email.save();
                console.log(`Worker 5: Updated email ${email.email} to status: ${email.status}`);
                
                // Small delay between verifications to avoid rate limiting
                await new Promise(resolve => setTimeout(resolve, 200));
            } catch (err) {
                console.error(`Worker 5: Error processing email ${email.email}:`, err);
            }
        }
    } catch (err) {
        console.error('Worker 5: Error in batch processing:', err);
    }
}
...
console.log('Starting Worker 5: continuous email verification worker with credit management...');
verifyPendingEmails();

// Handle graceful shutdown
process.on('SIGINT', async () => {
    console.log('Stopping Worker 5: email verification worker...');
